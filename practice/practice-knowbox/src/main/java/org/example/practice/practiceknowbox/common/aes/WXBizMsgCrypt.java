 ...
 */
package org.example.practice.practiceknowbox.common.aes;
...
public class WXBizMsgCrypt {
     ...
     *             执行失败，请查看该异常的错误码和具体的错误信息
     */
    public String VerifyURL(String msgSignature, String timeStamp, String nonce, String echoStr, int platform)
        throws AesException {
        String echo = platform == 1 ? "" : echoStr;
        String signature = SHA1.getSHA1(token, timeStamp, nonce, echo);
        if (!signature.equals(msgSignature)) {
            throw new AesException(AesException.ValidateSignatureError);
        }
        return platform == 1 ? echoStr : this.decrypt(echoStr);
    }
