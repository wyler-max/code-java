package org.example.practice.practicecode.javalang.utils.collection.list.linkedlist;
...
/**
 * LinkedList 是一个双向链表，在内存中是不连续的（JDK1.7之前的版本是环形链表，JDK1.7之后是直线型双向链表） LinkedList实现了Queue、Deque接口 对内存利用率高，由于是链表，插入时较快
 *
 * 使用场景： 1、增删多，读少 2、不存在RandomAccess时 综上，还是尽量使用ArrayList 代替LinkedList
 */
