...
public class ListSortTest {
    ...
    public static void listSort1(List<UserInfo> userInfoList) {
        // 按波动降序--绝对值
        List<UserInfo> sortedList1 = userInfoList.stream()
            .sorted(Comparator.comparing(a -> Double.valueOf(Math.abs(a.getDeviation())))).collect(Collectors.toList());
        Collections.reverse(sortedList1);
        // sortedList1.forEach(System.out::println);

        // 按年龄降序
        List<UserInfo> sortedList2 = userInfoList.stream().sorted(Comparator.comparing(UserInfo::getAge).reversed())
        ...
        System.out.println("==================");
        List<UserInfo> sortedList3 = userInfoList.stream()
            .sorted(Comparator.comparing(UserInfo::getAge).reversed().thenComparing(UserInfo::getBirthday).reversed())
            .collect(Collectors.toList());
    ...
    public static void listSort2(List<OrderInfo> orderInfoList) {
        ...
        // stream().sorted()排序
        List<OrderInfo> sortedOrderList =
            orderInfoList.stream().sorted(Comparator.comparing(OrderInfo::getStatus, (x, y) -> {
    ...
    public static void main(String[] args) {
        ...
        List<OrderInfo> orderInfoList =
            Lists.newArrayList(new OrderInfo(1001L, "待支付", 200, "2020-09-01"), new OrderInfo(1010L, "待支付", 300, null),
                new OrderInfo(1002L, "待支付", 300, "2020-09-03"), new OrderInfo(1008L, "待支付", 300, null),
                new OrderInfo(1009L, "待支付", 300, "2020-09-06"), new OrderInfo(1006L, "已过期", 400, "2020-09-02"),
                new OrderInfo(1003L, "待支付", 400, "2020-09-03"), new OrderInfo(1007L, "已取消", 300, "2020-09-03"),
                new OrderInfo(1005L, "已购买", 400, "2020-09-04"), new OrderInfo(1004L, "已购买", 200, "2020-09-01"));
