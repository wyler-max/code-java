package org.example.practice.practicecode.javalang.utils.collection.list.arraylist;
 ...
 *
 * 1、 2、继承了RandomAccess 接口，读取快 3、使用尾插法+良好的扩容机制（minCapacity > 1.5? minCapacity: 1.5）使得ArrayList的插入效率不亚于LinkedList
 * 4、remove时，最终会执行System.arraycopy()拷贝被删除之后的元素，所以效率可能不如LinkedList 5、private transient Object[] elementData; 是因为数组的
 * 容量!=空间 ，为节省空间，不允许序列化整个数组空间，而是在序列化的时候，另外处理
 *
 * 共性： 1、非线程安全的 2、可以包含重复的元素 3、维护了元素插入时的顺序
 */
